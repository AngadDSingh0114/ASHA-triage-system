...
// IMNCI §3 (Ear Problem)
//
// Mastoid swelling → MASTOIDITIS (RED)
// Ear pain or discharge → ACUTE EAR INFECTION (YELLOW)
...
export function classifyEarProblem(
  ...
  // Guard: only classify if ear pain/discharge or mastoid swelling is present
  if (!s.ear_pain_or_discharge && !s.mastoid_swelling) return null;

  if (s.mastoid_swelling) {
    return {
      name: "ear problem",
...
export function classify(i: TriageInput): TriageResult {
  ...
  // 5. Fail-safe: if nothing fired and no data was entered → RED
  if (conditions.length === 0 && dangerSigns.length === 0) {
    // Check if symptoms object has any explicitly entered data
    const hasEnteredData = Object.values(i.symptoms).some(
