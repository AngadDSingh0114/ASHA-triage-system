...
describe("Fail-safe", () => {
  // Test 18: Empty symptoms → YELLOW
  test("#18 — empty symptoms → YELLOW (insufficient data)", () => {
  ...
  test("all symptoms explicitly false → YELLOW", () => {
    const input = makeInput({
      age_months: 12,
      symptoms: {
        cough_or_difficulty_breathing: false,
    ...
    const result = classify(input);
    // No condition fires → fail-safe to YELLOW
    expect(result.flag).toBe("yellow");
  });
});
